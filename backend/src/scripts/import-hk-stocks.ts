...
async function importHKStocks() {
  const options = parseArguments();
  const taskId = `hk-import-${Date.now()}`;

  console.log('🇭🇰 港股数据导入工具\n');
  console.log('配置:');
  ...
  try {
    ...
    const { constituents, stats: indexStats } = await indexService.fetchHKConstituents();

    console.log(`\n✅ 获取到 ${constituents.length} 只唯一港股`);
    ...
    if (options.resume) {
      ...
      const hkCheckpoint = activeCheckpoints.find((c) => c.market === 'HK');

      if (hkCheckpoint) {
        checkpoint = await checkpointTracker.resumeCheckpoint(hkCheckpoint.taskId);
        importedStockCodes = await checkpointTracker.getImportedStocks(hkCheckpoint.taskId);
        console.log(`✅ 恢复任务 ${hkCheckpoint.taskId}`);
        console.log(`   已导入: ${importedStockCodes.length}/${hkCheckpoint.totalStocks}`);
        ...
        try {
          ...
          if (!options.dryRun) {
            ...
            const klineData = klineResult.data.map((k) => ({
              ...
              volume: k.volume,
              amount: k.amount,
              source: klineResult.source || 'akshare',
            }));
    ...
    if (stats.errors.length > 0) {
      ...
      const logFile = path.join(
        logDir,
        `import-hk-stocks-${stats.startTime.toISOString().split('T')[0]}.json`,
      );
