...
export async function configureSQLite(prisma: PrismaClient): Promise<void> {
  try {
    ...
    // 记录当前配置
    const config = await (prisma.$queryRawUnsafe as Function)('PRAGMA compile_options;');
...
export async function getDatabaseStats(prisma: PrismaClient): Promise<{
  ...
  const qr = prisma.$queryRawUnsafe as (q: string) => Promise<any[]>;
  const [journalMode] = await qr('PRAGMA journal_mode;');
  const [pageSize] = await qr('PRAGMA page_size;');
  const [cacheSize] = await qr('PRAGMA cache_size;');
  const [synchronous] = await qr('PRAGMA synchronous;');
  const walCheckpoint = await qr('PRAGMA wal_checkpoint;');
