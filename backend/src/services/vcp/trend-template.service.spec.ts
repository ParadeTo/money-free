...
describe('TrendTemplateService', () => {
  ...
  it('check price > MA150', () => {
    ...
    expect(
      service
        .runAllChecks({ ...passingInput, currentPrice: 80 })
        .checks.find((c) => c.name === 'priceAboveMA150')?.pass,
    ).toBe(false);
  });
  ...
  it('check price > MA200', () => {
    const check = service
      ...
      .checks.find((c) => c.name === 'priceAboveMA200');
    expect(check?.pass).toBe(true);
    expect(
      service
        .runAllChecks({ ...passingInput, currentPrice: 75 })
        .checks.find((c) => c.name === 'priceAboveMA200')?.pass,
    ).toBe(false);
  });
  ...
  it('check MA150 > MA200', () => {
    const check = service
      .runAllChecks(passingInput)
      .checks.find((c) => c.name === 'ma150AboveMA200');
    expect(check?.pass).toBe(true);
    expect(
      service
        .runAllChecks({ ...passingInput, ma150: 75 })
        .checks.find((c) => c.name === 'ma150AboveMA200')?.pass,
    ).toBe(false);
  });
  ...
  it('check MA50 > MA150', () => {
    const check = service
      .runAllChecks(passingInput)
      .checks.find((c) => c.name === 'ma50AboveMA150');
    expect(check?.pass).toBe(true);
    expect(
      service
        .runAllChecks({ ...passingInput, ma50: 80 })
        .checks.find((c) => c.name === 'ma50AboveMA150')?.pass,
    ).toBe(false);
  });
  ...
  it('check MA50 > MA200', () => {
    const check = service
      .runAllChecks(passingInput)
      .checks.find((c) => c.name === 'ma50AboveMA200');
    expect(check?.pass).toBe(true);
    expect(
      service
        .runAllChecks({ ...passingInput, ma50: 75 })
        .checks.find((c) => c.name === 'ma50AboveMA200')?.pass,
    ).toBe(false);
  });
  ...
  it('check price > MA50', () => {
    const check = service
      .runAllChecks(passingInput)
      .checks.find((c) => c.name === 'priceAboveMA50');
    expect(check?.pass).toBe(true);
    expect(
      service
        .runAllChecks({ ...passingInput, currentPrice: 90 })
        .checks.find((c) => c.name === 'priceAboveMA50')?.pass,
    ).toBe(false);
  });
  ...
  it('check ≥25% above 52wk low', () => {
    const check = service
      .runAllChecks(passingInput)
      .checks.find((c) => c.name === 'above25PctFrom52WkLow');
    expect(check?.pass).toBe(true);
    expect(
      service
        .runAllChecks({ ...passingInput, currentPrice: 70 })
        .checks.find((c) => c.name === 'above25PctFrom52WkLow')?.pass,
    ).toBe(false);
  });
  ...
  it('check within 25% of 52wk high', () => {
    const check = service
      .runAllChecks(passingInput)
      .checks.find((c) => c.name === 'within25PctOf52WkHigh');
    expect(check?.pass).toBe(true);
    expect(
      service
        .runAllChecks({ ...passingInput, currentPrice: 80 })
        .checks.find((c) => c.name === 'within25PctOf52WkHigh')?.pass,
    ).toBe(false);
  });
  ...
  it('check RS ≥ 70', () => {
    const check = service
      .runAllChecks(passingInput)
      .checks.find((c) => c.name === 'rsRatingAbove70');
    expect(check?.pass).toBe(true);
    expect(
      service
        .runAllChecks({ ...passingInput, rsRating: 60 })
        .checks.find((c) => c.name === 'rsRatingAbove70')?.pass,
    ).toBe(false);
  });
