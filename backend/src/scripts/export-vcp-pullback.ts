...
async function main() {
  ...
  try {
    ...
    const filename =
      filterType === 'pullback' ? `VCP选股-浅回调-${dateStr}.md` : `VCP选股-浅收缩-${dateStr}.md`;
    ...
    console.log(
      `筛选类型: ${filterType === 'pullback' ? '回调幅度 < 10%' : '收缩幅度 < 10%（无回调）'}`,
    );
    ...
    if (filterType === 'pullback') {
      console.log(
        '排名 | 股票代码 | 股票名称       | 回调幅度% | 回调天数 | 距最低点   | 反弹%    | RS评分 | 收缩% | 距52周高%',
      );
      console.log('-'.repeat(120));
      ...
      console.log(
        '排名 | 股票代码 | 股票名称       | 收缩幅度% | 收缩次数 | RS评分 | 成交量萎缩 | 最新价  | 涨跌幅% | 距52周高%',
      );
      console.log('-'.repeat(120));
