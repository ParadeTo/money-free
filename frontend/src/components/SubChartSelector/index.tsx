import { Select, Checkbox } from 'antd';
import { useChartStore } from '../../store/chart.store';
import styles from './SubChartSelector.module.css';

export function SubChartSelector() {
  const { 
    showMA, 
    subChart1Indicator, 
    subChart2Indicator,
    setShowMA,
    setSubChart1Indicator, 
    setSubChart2Indicator 
  } = useChartStore();

  return (
    <div className={styles.container}>
      <div className={styles.section}>
        <Checkbox 
          checked={showMA}
          onChange={(e) => setShowMA(e.target.checked)}
        >
          显示均线
        </Checkbox>
      </div>

      <div className={styles.section}>
        <div className={styles.label}>副图1</div>
        <Select
          value={subChart1Indicator}
          onChange={setSubChart1Indicator}
          style={{ width: '100%' }}
          size="small"
        >
          <Select.Option value="none">不显示</Select.Option>
          <Select.Option value="rsi">RSI 指标</Select.Option>
          <Select.Option value="kdj">KDJ 指标</Select.Option>
        </Select>
      </div>

      <div className={styles.section}>
        <div className={styles.label}>副图2</div>
        <Select
          value={subChart2Indicator}
          onChange={setSubChart2Indicator}
          style={{ width: '100%' }}
          size="small"
        >
          <Select.Option value="none">不显示</Select.Option>
          <Select.Option value="volume">成交量</Select.Option>
          <Select.Option value="amount">成交额</Select.Option>
        </Select>
      </div>
    </div>
  );
}
